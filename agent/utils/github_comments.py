...
NATIVE_SWE_TAGS = ("@native-swe", "@nativeswe")
...
async def fetch_pr_comments_since_last_tag(
    ...
    """Fetch all PR comments/reviews since the last @native-swe tag.

    Fetches from all 3 GitHub comment sources, merges and sorts chronologically,
    then returns every comment from the last @native-swe mention onwards.
    ...
    Returns:
        List of comment dicts ordered chronologically from last @native-swe tag.
    """
    owner = repo_config.get("owner", "")
    repo = repo_config.get("name", "")
    headers = {
    ...
    tag_indices = [
        ...
        if any(tag in (comment.get("body") or "").lower() for tag in NATIVE_SWE_TAGS)
    ]

    if not tag_indices:
        return []

    # If this is the first @nativeswe invocation (only one tag), return ALL
