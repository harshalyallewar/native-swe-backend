...
def _route_check_queue_dispatch(state: AgentState) -> str:
    """
    Route after check_queue:
    - fatal_error → notify
    - has_new_instructions → classify (re-plan with appended todos)
    - no intent yet → classify (initial pass)
    - intent == question → notify (answer already generated by answer_qa)
    - steps remaining in plan → execute_step (keep executing)
    - all steps done → verify (only after ALL steps complete)
    """
    ...
    # For code_change and review: check if steps remain
    plan = state.get("plan") or []
    current_step = state.get("current_step", 0)
    if current_step < len(plan):
        return "execute_step"  # more steps to do

    return "verify"  # all steps done, now verify
