...
This guide walks you through setting up Native-SWE end-to-end: local development, GitHub App creation, LangSmith configuration, webhooks, and production deployment.
...
```bash
git clone https://github.com/langchain-ai/native-swe.git
cd native-swe
uv venv
...
Native-SWE authenticates as a [GitHub App](https://docs.github.com/en/apps/creating-github-apps) to clone repos, push branches, and open PRs.
...
2. Fill in:
   - **App name**: `native-swe` (or your preferred name)
   - **Homepage URL**: This can be any valid URL — it's only shown on the GitHub Marketplace page (which you won't be using). Use something like `https://github.com/langchain-ai/native-swe`
...
3. Choose which repositories Native-SWE should have access to
4. Click **Install**
...
Native-SWE can be triggered from GitHub, Linear, and/or Slack. **Configure whichever surfaces your team uses — you don't need all of them.**

### GitHub

GitHub triggering works automatically once your GitHub App is set up (step 3). Users can:
- Tag `@nativeswe` in issue titles or bodies to start a task
- Tag `@nativeswe` in issue comments for follow-up instructions
- Tag `@nativeswe` in PR review comments to have it address review feedback
...
Native-SWE listens for Linear comments that mention `@nativeswe`.
...
2. Name it `native-swe`, select **All access**, and copy the key
3. Save it as `LINEAR_API_KEY`

**Configure team-to-repo mapping:**

Native-SWE routes Linear issues to GitHub repos based on the Linear team and project. Edit the mapping in `agent/utils/linear_team_repo_map.py`:
...
Users can also override the team/project mapping per-comment by including `repo:owner/name` (or a GitHub URL) in their `@nativeswe` comment. The mapping is used as a fallback when no repo is specified in the comment text.
...
```json
{
    "display_information": {
        "name": "Native-SWE",
        "description": "Enables Native-SWE to interact with your workspace",
...
- `SLACK_BOT_USERNAME`: the bot's display name (e.g. `native-swe`)
...
2. Create or comment on an issue with: `@nativeswe what files are in this repo?`
3. You should see:
...
2. Add a comment: `@nativeswe what files are in this repo?`
3. You should see:
   - A 👀 reaction on your comment within a few seconds
   - A new run in your LangSmith project
   - The agent replies with a comment on the issue
...
2. Mention the bot: `@native-swe what's in the repo?`
3. You should see:
...
- For GitHub: ensure the comment or issue contains `@nativeswe` (case-insensitive), and the commenter's GitHub username is in `GITHUB_USER_EMAIL_MAP`
- For Linear: ensure the comment contains `@nativeswe` (case-insensitive)
