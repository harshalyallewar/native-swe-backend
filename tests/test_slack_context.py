...
def test_strip_bot_mention_removes_bot_username_tag() -> None:
    assert (
        strip_bot_mention("@native-swe please check", "UBOT", bot_username="native-swe")
        == "please check"
    )
...
def test_replace_bot_mention_with_username() -> None:
    assert (
        replace_bot_mention_with_username("<@UBOT> can you help?", "UBOT", "native-swe")
        == "@native-swe can you help?"
    )
...
def test_format_slack_messages_for_prompt_replaces_bot_id_mention_in_text() -> None:
    formatted = format_slack_messages_for_prompt(
        ...
        {"U123": "alice"},
        bot_user_id="UBOT",
        bot_username="native-swe",
    )

    assert formatted == "@alice(U123): @native-swe status update?"


def test_select_slack_context_messages_detects_username_mention() -> None:
    selected, mode = select_slack_context_messages(
        [
            {"ts": "1.0", "text": "@native-swe first request", "user": "U1"},
            {"ts": "2.0", "text": "follow up", "user": "U2"},
            {"ts": "3.0", "text": "@native-swe second request", "user": "U3"},
        ],
        "3.0",
        bot_user_id="UBOT",
        bot_username="native-swe",
...
def test_get_slack_repo_config_repo_name_only_space_syntax(
    ...
    repo = asyncio.run(webapp.get_slack_repo_config("fix bug in repo native-swe", "C123", "1.234"))

    assert repo == {"owner": "langchain-ai", "name": "native-swe"}
